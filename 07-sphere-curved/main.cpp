...
const string world_fname = "resources/07-sphere-curved/world.urdf";
const string object_fname = "resources/07-sphere-curved/sphere_object.urdf";
const string object1_name = "Sphere1";

const double sphere_radius = 0.15;
const string object_link_name = "object";
Affine3d sphere1_in_world;

const string plane_name = "Plane1";
const string plane_link_name = "table";
const string plane_fname = "resources/07-sphere-curved/plane_tilting.urdf";

const double experiment_runtime = 20.0; // sec
...
Logging::Logger logger(10000, "log_curvature.csv");
...
int main(int argc, char** argv) {
    ...
    sphere1_in_world.translation() = sai2_sim->_world->getBaseNode(object1_name)->getLocalPos().eigen();
    sphere1_in_world.linear() = sai2_sim->_world->getBaseNode(object1_name)->getLocalRot().eigen();
    // cout << box1_in_world.translation().transpose() << endl;

    Affine3d plane_in_world;
    plane_in_world.translation() = sai2_sim->_world->getBaseNode(plane_name)->getLocalPos().eigen();
    plane_in_world.linear() = sai2_sim->_world->getBaseNode(plane_name)->getLocalRot().eigen();
    plane_in_world.translation() += plane_in_world.linear().col(2)*0.1/2.0; // z-height of box
    ...
    // load objects
    auto coobject1 = new Sai2Model::Sai2Model(object_fname, false, sphere1_in_world, sphere1_in_world.linear().transpose()*grav_vector);

    // add some initial velocity. TODO: move to parser
    // coobject1->_dq[4] = 10.0;

    // tilting table object
    auto coobject2 = new Sai2Model::Sai2Model(plane_fname, false, plane_in_world, plane_in_world.linear().transpose()*grav_vector);

    // set up COP sim world
    auto cop_sim = new Sai2COPSim::COPSimulator(friction, restitution);

    //TODO: test with same primitive name
    cop_sim->addSphereObject(object1_name, object_link_name, "sphere1", coobject1, sphere_radius);

    // add plane
    cop_sim->addPlaneObject(plane_name, plane_link_name, "plane1", coobject2, plane_in_world.linear().col(2), plane_in_world.translation());

    // -- CHOOSE POINT CONTACT VS COP FOR STEADY STATE RESOLUTION --
    cop_sim->_contact_model->setSupportPtContactSteadyContact(false);
    ...
    while (!glfwWindowShouldClose(window) && fSimulationRunning) {
        ...
        graphics->updateGraphics(plane_name, coobject2);
...
void simulation(Sai2COPSim::COPSimulator* sim) {
    ...
    while (fSimulationRunning && last_time < experiment_runtime) {
        ...
        try {
            if(curr_time < 3) {
                desired_orientation = Eigen::AngleAxisd(0.1*sin(2.0*M_PI*traj_time/4.0),
                                                             Vector3d::UnitX()).toRotationMatrix();
                traj_time += dt;
            } else if(curr_time > 3) {
                int div = (traj_time - 3) / 2;
                // cout << abs((traj_time - 3) - 2*div) << " " << loop_dt << endl;
                if( abs((traj_time - 3) - 2*div) < dt && hold_time < 2) {
                    hold_time += dt;
                } else {
                    hold_time = 0;
                    desired_orientation = Eigen::AngleAxisd(0.1*sin(2.0*M_PI*traj_time/4.0),
            ...
            plane->_model->rotation(plane_orientation, plane_link_name);
            Sai2Model::orientationError(orientation_error, desired_orientation, plane_orientation);
            // apply plane force
            plane->jtau_act = -150.0*orientation_error - 20.0*plane->_model->_dq;


            sim->integrate(loop_dt);
            if(!sim->_contact_map._islands.empty()) {
                penetration << sim->_contact_map._islands[0]._contact_prim_pairs.front().info->min_distance,
                    0, 0;
            } else {
                penetration << -1, 0, 0;
            }
            penetration[1] = sim->_iterations*dt; // forgive the hack here to store sim iterations in this log buffer

            if(timer.elapsedCycles() % 10000 == 0) {
                plane->_model->updateModel();
                plane->_model->rotation(plane_orientation, plane_link_name);
                cout << "normal acceleration "
                    << plane_orientation.col(2).dot(object->_model->_ddq.segment<3>(0))
                    << " penetration " << sim->_max_penetration_current
                    << " num active contacts " << sim->_contact_model->_contact_island_models[0]._active_contacts.size()
                    << endl;
            }
