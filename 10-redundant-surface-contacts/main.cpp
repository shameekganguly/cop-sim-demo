...
const string world_fname = "resources/10-redundant-surface-contacts/world.urdf";
const string object_fname = "resources/10-redundant-surface-contacts/box_object.urdf";
const string object1_name = "Box1";
const double box_xlength = 0.2;
const double box_ylength = 0.1;
const double box_zlength = 0.2;
const Vector3d box_left_pos(0.0, -0.2, 0.0);
const Vector3d box_right_pos(0.0, 0.2, 0.0);
const string object_link_name = "object";
const string plane_name = "Ground";
Affine3d box1_in_world;
...
int main(int argc, char** argv) {
    ...
    box1_in_world.translation() = sai2_sim->_world->getBaseNode(object1_name)->getLocalPos().eigen();
    box1_in_world.linear() = sai2_sim->_world->getBaseNode(object1_name)->getLocalRot().eigen();
    ...
    // load objects
    auto coobject1 = new Sai2Model::Sai2Model(object_fname, false, box1_in_world, box1_in_world.linear().transpose()*grav_vector);
    ...
    cop_sim->addObject(object1_name, coobject1);

    //TODO: test with same primitive name
    Affine3d box_left_tf = Affine3d::Identity();
    box_left_tf.translation() = box_left_pos;
    cop_sim->addBoxToObject(object1_name,
                        object_link_name,
                        "box_left",
                        box_xlength,
                        box_ylength,
    ...
    Affine3d box_right_tf = Affine3d::Identity();
    box_right_tf.translation() = box_right_pos;
    cop_sim->addBoxToObject(object1_name,
                        object_link_name,
                        "box_right",
                        box_xlength,
    ...
    // Enable/ Disable steady state contact resolution with point contacts
...
void simulation(Sai2COPSim::COPSimulator* sim) {
    ...
    while (fSimulationRunning && timer.elapsedTime() < 5) {
        ...
        try {
            ...
            // std::cout << object->_model->_q.transpose() << std::endl;
